 ...
 *
 */
function placeBrands(brands) {
    var _brands = _.groupBy(brands, 'category.id');
    console.log(_brands);
    for (var entry in _brands)
        _brands[entry.id].forEach(function (item) {
            marker = new google.maps.Marker({
                position: new google.maps.LatLng(item.location_latitude, item.location_longitude),
                map: mapObject,
                icon: 'assets/pins/' + entry + '.png',
            });
...
(function () {
    ...
    map.addListener('idle', function () {
        App.Stores.loadData({
            ...
            query: App.Helpers.getMapQuery(map)
        }).then(function (list) {
            placeBrands(list);
        })
    });
    App.Stores.loadData({
        ...
        var $list = $('#map_filter').find('ul');
        _.each(list.categories, function (entry) {
            console.log(entry);
            $list.append($('<li><a href="javascript:toggleMarkers(\'' + entry.id + '\');">' +
                '<i class="' + entry.icon_class_name + '"></i><span>' + entry.name + '</span></a></li>'));
        });
        /*
